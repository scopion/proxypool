...
# yum update -y
# yum install squid -y
wget http://dailichi.oss-cn-hangzhou.aliyuncs.com/libecap-1.0.0-3.el7.centos.x86_64.rpm
yum localinstall libecap-1.0.0-3.el7.centos.x86_64.rpm -y
wget http://dailichi.oss-cn-hangzhou.aliyuncs.com/squid-3.5.20-1.el7.centos.x86_64.rpm
yum localinstall squid-3.5.20-1.el7.centos.x86_64.rpm -y
# config squid
...
echo "1 0 * * * tail -100 /app/proxy-pool-slave/nohup.out >> /app/proxy-pool-slave/nohup.out.backup && echo ' '> /app/proxy-pool-slave/nohup.out"  >>/var/spool/cron/root
